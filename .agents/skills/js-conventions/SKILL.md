---
name: js-conventions
description: Apply JavaScript style. Use when writing or editing .js, .mjs, .ts, .jsx, .tsx files (with certain corrections for typescript), formatting code, or when the user asks about code style or linting.
---
...
- Use more modern JavaScript syntax: `array.at(-1)` instead of `array[array.length - 1]`, `array.toSorted(...)` instead of `array.sort(...)`, and so on.
- Generate TypeScript code using the principle 'invalid state must be unrepresentable'. Prefer discriminated unions over optional fields, avoid null/undefined, and encode all invariants in types rather than runtime checks.
...
## Style
- Use an empty line to separate logical blocks of code (e.g., between function definitions, variable declaration groups, different components usage, before if/return statements, and after import statements, etc.)